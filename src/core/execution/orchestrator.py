...
class UnifiedOrchestrator:
    ...
    async def execute(self, query: str, task_id: str, emit_event: Any) -> Dict[str, Any]:
        ...
        is_data_task = any(kw in lower_query for kw in data_keywords)
        ...
        if is_data_task:
            ...
            content = result_pkg.get("content", "")
            return {
                "success": "✅" in content or "Success" in content or "executed" in content.lower(),
                "summary": content,
                "intermediate_steps": [{
                    "type":      "Action",
                    "content":   content,
                    "timestamp": datetime.now().strftime("%I:%M:%S %p"),
                    "tool":      result_pkg.get("tool_used", "Data Retriever"),
        ...
        is_web_task = has_url or any(kw in lower_query for kw in web_keywords)
